...
const chartOpts = (ylabel = "") => ({
  ...
  scales: {
    ...
    y: {
      grid: { color: "#f0f4f1" },
      ticks: { font: { size: 10 }, callback: v => ylabel ? ylabel + v.toLocaleString() : v },
    },
  },
});
    ...
    <div className="rp-overlay" onClick={onClose}>
      <div className="rp-modal rp-modal-lg" onClick={e => e.stopPropagation()}>
        ...
        <div className="rp-modal-body">
          {loading ? (
            <div className="rp-loading">Loading report...</div>
          ) : !data ? (
            <div className="rp-no-data">Failed to load.</div>
          ) : (
            <>
...
export default function Reports() {
  ...
  // ── UPDATED: all year-sensitive APIs now pass selectedYear ────────────────
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const results = await Promise.allSettled([
          getOverviewAPI(selectedYear),           // ← year filter
          getMonthlyCollectionAPI(selectedYear),  // ← year filter
          getLoanStatusAPI(selectedYear),         // ← year filter
          getLoanTypeAPI(selectedYear),           // ← year filter
          getPaymentBehaviorAPI(selectedYear),    // ← year filter
          getAuditLogAPI(),
          getClassificationAPI(selectedYear),
          getMemberPerformanceAPI(selectedYear),
          getTopBorrowersAPI(selectedYear),
          getLoanDistributionAPI(),
          getYearlyComparisonAPI(),
          getShareCapitalAPI(),
          getOverdueAnalysisAPI(),
    ...
    load();
  }, [selectedYear]); // ← re-fetch every time year changes
  ...
  const clsBarOpts = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: true, position: "top" } }, scales: { x: { grid: { display: false } }, y: { grid: { color: "#f0f4f1" }, ticks: { callback: v => v + "%" }, max: 100 } } };
  ...
  return (
    <div className="rp-wrapper">
      {showPreview && (
        <ReportPreviewModal
          type={reportType} dateFrom={dateFrom} dateTo={dateTo}
      ...
      <div className="rp-page-header">
        ...
        <div className="rp-year-selector">
          ...
          <select
            ...
            style={{ padding: "4px 10px", borderRadius: 8, border: "1px solid #ddd", fontSize: 12, fontFamily: "inherit" }}
          >
            ...
            <>
              <div className="rp-kpi-grid">
                {OVERVIEW_CARDS.map((c, i) => (
                  <div key={i} className={`rp-kpi-card ${c.color || ""}`}>
                    <div className="rp-kpi-icon" style={{ background: c.bg, borderRadius: 10, width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {c.icon}
                    </div>
              ...
              <div className="rp-quick-charts" style={{ marginTop: 16 }}>
                <div className="rp-chart-card rp-chart-wide">
                  ...
                  <div style={{ height: 220 }}>
                    {monthly.length === 0
                      ? <div className="rp-no-data">No data yet.</div>
                ...
                <div className="rp-chart-card">
                  ...
                  <div style={{ height: 220 }}>
                    {monthlyLoans.length === 0
                      ? <div className="rp-no-data">No data yet.</div>
                      : <Line data={monthlyLoansData} options={chartOpts()}/>
                    }
                  </div>
                </div>
              </div>
              ...
              <div className="rp-chart-card" style={{ marginTop: 16 }}>
                ...
                <div style={{ height: 220 }}>
                  {yearlyComp.length === 0
                    ? <div className="rp-no-data">No data yet.</div>
                    : <Line data={yearlyLineData} options={lineOpts}/>
                  }
                </div>
              </div>
            </>
        ...
        <div className="rp-overview">
          <div className="rp-chart-card" style={{ marginBottom: 16 }}>
            ...
            <div style={{ height: 260 }}>
              {classification.length === 0 ? <div className="rp-no-data">No data.</div> : <Bar data={clsBarData} options={clsBarOpts}/>}
            </div>
          </div>
            ...
            {classification.map((c, i) => (
              <div key={i} className="rp-cls-card">
                <div className="rp-cls-icon">
                  {c.classification === "Student" ? <span style={{ fontSize: 28 }}>🎓</span>
                    : c.classification === "Senior" ? <span style={{ fontSize: 28 }}>👴</span>
                ...
                <div className="rp-cls-bar-wrap">
                  <div className="rp-cls-bar" style={{ width: `${c.on_time_rate}%`, background: c.on_time_rate >= 80 ? "#2e7d32" : "#f57c00" }}/>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="rp-chart-card">
            ...
            <div className="rp-preview-table-wrap">
              <table className="rp-preview-table">
                <thead>
                  <tr><th>#</th><th>Member ID</th><th>Name</th><th>Classification</th><th>Loans</th><th>Total Amount</th><th>Share Capital</th><th>Savings Balance</th></tr>
                </thead>
                <tbody>
        ...
        <div className="rp-overview">
          ...
          <div className="rp-chart-card">
            <div className="rp-preview-table-wrap">
              <table className="rp-preview-table">
                <thead>
                  <tr>
                    <th>Rank</th><th>Member ID</th><th>Name</th><th>Classification</th>
                    <th>Payments</th><th>On Time</th><th>Late</th><th>Overdue</th>
                    <th>On-Time Rate</th><th>Total Paid</th><th>Savings</th><th>Rating</th>
                  </tr>
                </thead>
                <tbody>
                  {memberPerf.length === 0 ? (
                    ...
                    <tr key={i}>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <span style={{ fontWeight: 700, color: m.on_time_rate >= 80 ? "#1b5e20" : "#f57c00" }}>{m.on_time_rate}%</span>
                      </td>
          ...
          <div className="rp-chart-card" style={{ marginTop: 16 }}>
            ...
            <div className="rp-preview-table-wrap">
              <table className="rp-preview-table">
                <thead>
                  <tr><th>#</th><th>Member ID</th><th>Name</th><th>Classification</th><th>Loans Taken</th><th>Total Loaned</th><th>Share Capital</th><th>Max Loanable</th><th>Savings Balance</th></tr>
                </thead>
                <tbody>
        ...
        <div className="rp-audit">
          ...
          <div className="rp-audit-table-wrap">
            <table className="rp-audit-table">
              ...
              <tbody>
                {auditLog.length === 0 ? (
                  <tr><td colSpan={9} style={{ textAlign: "center", padding: "20px", color: "#aaa" }}>No audit log yet.</td></tr>
                ) : auditLog.map((row, i) => (
                  <tr key={i}>
                    <td>{row.paid_at}</td>
