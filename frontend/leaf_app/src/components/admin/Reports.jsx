...
import { BarChart2, TrendingUp, FileText, Link2, Users, PieChart } from "lucide-react";
import "./Reports.css";
...
const REPORT_TYPES = [
  ...
  "Payment Behavior","Blockchain Audit Log","Member Performance Report","Classification Analytics",
];
...
export default function Reports() {
    ...
    const load = async () => {
      ...
      try {
        const results = await Promise.allSettled([
          ...
          getClassificationAPI(selectedYear), getMemberPerformanceAPI(selectedYear),
          getTopBorrowersAPI(selectedYear), getLoanDistributionAPI(),
          getYearlyComparisonAPI(), getShareCapitalAPI(),
          getOverdueAnalysisAPI(), getMonthlyLoansAPI(selectedYear),
        ]);
  ...
  const OVERVIEW_CARDS = [
    ...
    { icon:"🏦", val:`${overview.total_loans||0}`,       label:"Total Loans",        color:"blue" },
    ...
    <div className="rp-wrapper">
        ...
        <div className="rp-overview">
          {loading ? <div className="rp-loading">Loading...</div> : (
            <>
              ...
              {/* Monthly Collection + Monthly Loans side by side */}
      ...
      {activeTab === "classification" && (
        <div className="rp-overview">
          <div className="rp-chart-card" style={{marginBottom:16}}>
            ...
            <div className="rp-chart-sub">Blue collar vs White collar vs Student — sino ang mas madalas magbayad?</div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:16,marginBottom:16}}>
            {classification.map((c,i) => (
              <div key={i} className="rp-cls-card">
                <div className="rp-cls-icon">{c.classification==="Student"?"🎓":c.classification==="Senior"?"👴":"💼"}</div>
          ...
          <div className="rp-chart-card">
            ...
            <div className="rp-preview-table-wrap">
              <table className="rp-preview-table">
                <thead>
                  <tr><th>#</th><th>Member ID</th><th>Name</th><th>Classification</th><th>Loans</th><th>Total Amount</th><th>Share Capital</th></tr>
                </thead>
                <tbody>
                  {(topBorrowers.data||[]).length===0 ? (
                    <tr><td colSpan={7} style={{textAlign:"center",color:"#aaa",padding:20}}>No data yet.</td></tr>
      ...
      {activeTab === "performance" && (
        <div className="rp-overview">
              ...
              <table className="rp-preview-table">
                <thead>
                  <tr>
                    ...
                    <th>On-Time Rate</th><th>Total Paid</th><th>Rating</th>
                  </tr>
                </thead>
                <tbody>
                  {memberPerf.length===0 ? (
                    <tr><td colSpan={11} style={{textAlign:"center",color:"#aaa",padding:20}}>No payment data for this year.</td></tr>
                  ) : memberPerf.map((m,i) => (
                    <tr key={i}>
          ...
          <div className="rp-chart-card" style={{marginTop:16}}>
            <div className="rp-chart-title">Share Capital Growth (Top 20 Members)</div>
            <div className="rp-chart-sub">Paano lumalaki ang share capital ng bawat member sa bawat loan (3% ng loan amount)</div>
            <div className="rp-preview-table-wrap">
              <table className="rp-preview-table">
                <thead>
                  <tr><th>#</th><th>Member ID</th><th>Name</th><th>Classification</th><th>Loans Taken</th><th>Total Loaned</th><th>Share Capital</th><th>Max Loanable (×2)</th></tr>
                </thead>
                <tbody>
