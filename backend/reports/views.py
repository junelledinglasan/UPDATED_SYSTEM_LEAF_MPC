...
def classification_analytics_view(request):
    """
    Blue collar vs White collar vs Student vs Senior payment performance.
    Answers: Sino ang mas madalas magbayad — basurero vs government worker?
    """
    if request.user.role not in ['admin', 'staff']:
        return Response({'error': 'Unauthorized.'}, status=403)
    ...
    for cls in classifications:
        members_in_cls = Member.objects.filter(
            pre_member__classification=cls
        )
...
def member_performance_view(request):
    """
    Individual member payment performance ranking.
    Answers: Sino ang maganda/hindi maganda ang performance sa pagbabayad?
    """
    if request.user.role not in ['admin', 'staff']:
        return Response({'error': 'Unauthorized.'}, status=403)
...
def top_borrowers_view(request):
    """
    Members with most loans per year.
    Answers: Sino ang pinaka-maraming naka-loan this year?
    """
    if request.user.role not in ['admin', 'staff']:
        return Response({'error': 'Unauthorized.'}, status=403)
...
def loan_amount_distribution_view(request):
    """
    Loan amount distribution by range.
    Shows: how many loans fall in each amount bracket.
    """
    if request.user.role not in ['admin', 'staff']:
        return Response({'error': 'Unauthorized.'}, status=403)
    ...
    for label, min_amt, max_amt in ranges:
        ...
        total = float(Loan.objects.filter(amount__gte=min_amt, amount__lte=max_amt)
                      .aggregate(t=Sum('amount'))['t'] or 0)
        result.append({
            'range': label,
            'count': count,
...
def yearly_comparison_view(request):
    """
    Year-over-year comparison of collections, loans, and members.
    """
    if request.user.role not in ['admin', 'staff']:
        return Response({'error': 'Unauthorized.'}, status=403)
...
def share_capital_growth_view(request):
    """
    Share capital growth tracking — shows how members' share capital
    grows as they take loans (3% per loan).
    """
    if request.user.role not in ['admin', 'staff']:
        return Response({'error': 'Unauthorized.'}, status=403)
...
def overdue_analysis_view(request):
    """
    Overdue loan analysis — which classification has most overdue loans.
    """
    if request.user.role not in ['admin', 'staff']:
        return Response({'error': 'Unauthorized.'}, status=403)
    ...
    result = [
        {
            'classification': cls,
            'overdue_count':  v['count'],
            'total_overdue':  v['total_overdue'],
        }
